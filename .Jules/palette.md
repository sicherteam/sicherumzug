...
## 2026-08-01 - Layout-Level Keyboard Bypass Blocks via Skip Links
**Learning:** For keyboard and screen reader accessibility, adding a hidden "Skip to Content" link at the absolute top of the HTML DOM hierarchy (inside default layouts) is crucial to bypass redundant header/navigation blocks. Using high-contrast Tailwind transition styling on focus ensures visual confirmation for sighted keyboard navigators without cluttering the default UI.
**Action:** Always place a localized "Skip to Content" anchor immediately after the opening `<body>` tag, target `<main id="main">` with it, and style it with hidden-by-default, visible-on-focus Tailwind classes like `sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 focus:z-[100] focus:px-4 focus:py-2.5 focus:bg-primary-light focus:text-white focus:font-bold focus:rounded-xl focus:shadow-xl focus:ring-2 focus:ring-white transition-all duration-200`.
