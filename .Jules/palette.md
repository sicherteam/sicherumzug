...
## 2026-08-14 - Skip to Content Bypass Links & High-Contrast Navigation Focus Rings
**Learning:** High-contrast focus rings (`focus-visible:ring-2`) on custom megamenu buttons are essential for keyboard navigation, but they are incomplete without a "Skip to Content" bypass link at the top of the body. A skip link (`Zum Hauptinhalt springen`) must target `<main id="main">` with `sr-only focus:not-sr-only` to prevent keyboard and screen reader users from having to tab through dozens of navigation/dropdown list items on every page reload.
**Action:** Always include a skip-to-content link styled with a high-contrast brand color at the very top of `<body>`, and target `<main id="main" role="main">` with correct ID mappings. Pair it with unified header focus-visible rings using contrasting offset schemes (like `focus-visible:ring-amber-400 focus-visible:ring-offset-2`).
