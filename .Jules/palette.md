...
## 2026-08-17 - Accessible Clipboard Copy Action with Screen-Reader Announcements
**Learning:** Providing inline quick-copy buttons for contact details (like phone numbers and email addresses) reduces user effort and context switching, especially on desktop browsers. To ensure high accessibility, copy triggers must feature explicit `aria-label` tags, focus-visible rings (`focus-visible:ring-2`), visual icon feedback, and an `aria-live="polite"` element that announces successful copy actions to screen readers.
**Action:** Pair `data-copy-text` buttons with a hidden `aria-live="polite"` container (`#footer-copy-feedback`), swapping button icons to checkmarks for 2 seconds while updating the live region text upon copy.
